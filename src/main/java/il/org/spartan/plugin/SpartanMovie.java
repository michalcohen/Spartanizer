...
public class SpartanMovie extends AbstractHandler {
      ...
      progressService.run(false, true, new IRunnableWithProgress() {
        @Override public void run(final IProgressMonitor pm) {
          ...
          for (final ICompilationUnit currentCompilationUnit : compilationUnits) {
            close(page);
            IFile file = (IFile) currentCompilationUnit.getResource();
  ...
  static boolean focus(IWorkbenchPage p, IFile f) {
    try {
  ...
  static void close(IWorkbenchPage ¢) {
    ¢.closeAllEditors(true);
  }

  static boolean sleep(double i) {
    try {
      Thread.sleep((int) (1000 * i));
      return true;
    } catch (@SuppressWarnings("unused") InterruptedException __) {
      return false;
    }
  }
