...
public abstract class ModifierSort<N extends BodyDeclaration> extends Wring.ReplaceCurrentNode<N> implements Kind.Canonicalization {
  ...
  @SuppressWarnings("boxing") static boolean compare(final IExtendedModifier m1, final IExtendedModifier m2) {
    return MODIFIERS.get("" + m1) > MODIFIERS.get("" + m2);
  }

  @SuppressWarnings({ "boxing", "serial" }) static final Map<String, Integer> MODIFIERS = new HashMap<String, Integer>() {
    {
      put("public", 0);
      put("protected", 1);
      put("private", 2);
  ...
  private static void clearList(final List<IExtendedModifier> $) {
  ...
  private N go(final N $) {
    final List<IExtendedModifier> toSort = new ArrayList<>(step.modifiers($));
