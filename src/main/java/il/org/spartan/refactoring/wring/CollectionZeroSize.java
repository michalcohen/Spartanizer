...
public class CollectionZeroSize extends Wring.ReplaceCurrentNode<InfixExpression> implements Kind.DiscardRedundant {
  ...
  final List<InfixExpression.Operator> ao = Arrays.asList(new Operator[] { InfixExpression.Operator.EQUALS, InfixExpression.Operator.NOT_EQUALS, InfixExpression.Operator.GREATER });

  @Override ASTNode replacement(final InfixExpression e) {
    if (!e.getAST().hasResolvedBindings() || !(e.getLeftOperand() instanceof MethodInvocation) || !(e.getRightOperand() instanceof NumberLiteral) || !ao.contains(e.getOperator()))
      return null;
    ...
    final IMethodBinding iemb = BindingUtils.getVisibleMethod(mie != null ? mie.resolveTypeBinding() : BindingUtils.getClass(e), "isEmpty", null, e, compilationUnit);
