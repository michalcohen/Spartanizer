...
public interface iz {
  ...
  static boolean enhancedFor(final ASTNode ¢) {
    return !iz.not(¢, ENHANCED_FOR_STATEMENT);
  }
  ...
  static boolean not(final ASTNode n, final int type) {
  ...
  static boolean literal(final ASTNode ¢, final double d) {
    ...
    return token != null && LiteralParser.of(token) == type.Primitive.Certain.DOUBLE && iz.parsing.parsesTo(token, d);
  }
  ...
  static boolean literal(final ASTNode ¢, final int i) {
    ...
    if (numberLiteral == null)
      return false;
    final String token = numberLiteral.getToken();
    return token != null && LiteralParser.of(token) == type.Primitive.Certain.INT && iz.parsing.parsesTo(token, i);
  }
  ...
  static boolean literal(final ASTNode ¢, final long l) {
    ...
    if (numberLiteral == null)
      return false;
    final String token = numberLiteral.getToken();
    return token != null && LiteralParser.of(token) == type.Primitive.Certain.LONG && iz.parsing.parsesTo(token, l);
  }
  ...
  /** @param ¢
   * @return */
  static boolean singleVariableDeclaration(VariableDeclaration ¢) {
    return is(¢, SINGLE_VARIABLE_DECLARATION);
  }
}