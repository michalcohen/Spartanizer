...
@SuppressWarnings({ "unused" }) public interface Environment {
  /**
   * Information about a variable in the Environment - its {@link ASTNode}, his parent's, its {@link type},
   * and which other variables does it hide. 
   * this class is intentionally package level, and intenationally defined local. For now, clients should
  ...
  static class Information {
    ...
    /** For Information purposes, {@link type}s are equal if their key is equal.     */
    ...
    /**
     * @param ¢
     * @return true <b>iff</b> The the ASTNode (self) and its parent (blockScope) are the same ones, 
     *         the type's key() is the same, and if the Information nodes hidden are equal.
     */
    //Required for MapEntry equality, which is, in turn, required for Set containment check, which is required for testing.
  ...
  static LinkedHashSet<Entry<String, Information>> declaresDown(final ASTNode n) {
    ...
    n.accept(new ASTVisitor() {
      ...
      /*
       * Three groups of visitors here:
       * 1. Non-declarations with a name.
       * 2. Non-declarations without a name.
       * 3. Actual Declarations.
       * 
       * First two groups are those in which variable declarations can be made.
       * Since we want to be able to distinguish variables of different scopes, but with, perhaps, equal names, need to keep the scope.
       * The full scope might contain things that do not have a name, 
       * hence the need keep to visit ASTNodes without a name such as {@link Block}s, {@link ForStatement}s, etc.
       * 
       * Since there can be more than one such node in a parent, they are distinguished by their order of appearance. 
       * 
       * The third group is the one in which actual addition to the Environment is made.
       * 
       */
      ...
      Information createInformation(final VariableDeclarationFragment ¢, final type t) {
        //VariableDeclarationFragment, that comes from either FieldDeclaration, VariableDeclarationStatement or VariableDeclarationExpression,
        //does not contain its type. Hence, the type is sent from the parent in the convertToEntry calls.
        return new Information(¢.getParent(), getHidden(fullName(¢.getName())), ¢, t);
      }

      //Everything besides the actual variable declaration was visited for nameScope reasons. Once their visit is over, the nameScope needs to be restored.
       ...
       * (Dan Greenstein) lean towards searching 'declaresUp'. Current implementation only
