...
public class EnvironmentExample1 {
  void EX1() {
    ...
    s.equals("a");
    "a".equals(s);
    @NestedEnvironment({"EX1.s#String"})
    @FlatEnvironment({ "s" })
    int a = 0;
    out.print("a");
    @NestedEnvironment({ "EX1.a#int", "EX1.s#String" })
    @FlatEnvironment({ "a","s" })
    int b = 0;
    @Begin class A {}
    ++a;
    @End("a") class B {}
    @NestedEnvironment({ "EX1.a#int", "EX1.s#String" , "EX1.b#int" })
    @FlatEnvironment({ "a","s","b" })
    int c = 0;
    class Z {
      void g() {
        new Z() {
          int f(int a) {
            class Y {
              Y() {
                int q;
                @NestedEnvironment({"EX1.a#int", "EX1.s#String" , "EX1.b#int", "EX1.c#int", "EX1.Z.Y.q#int"})
                int d = 0;
                @Begin class A {}
                d = 3;
                @End("d") class B {}
              }
              @FlatEnvironment({"a","s","b","c"})
              int f; //not q!
            }
  ...
  public static class EX2 { // initializator
    ...
    @FlatEnvironment({"x","y"})
    static class C1{
      @NestedEnvironment({"EX2.C1.x#int"})
      @FlatEnvironment({"x"})
      public static int y; //no 'y' cause it is a static class (x is static also)
      C1 c1;
      @NestedEnvironment({"EX2.C1.x#int","EX2.C1.y#int","EX2.C1.c1#C1"})
      @FlatEnvironment({"x","y","c1"})
      public static int x;
  ...
  public static class EX3 { // hiding
    ...
    @NestedEnvironment({"EX3.x", "EX3.y"})
    @FlatEnvironment({"x","y"})
    static class x_hiding {
      @FlatEnvironment({})
      public static int x; 
      @NestedEnvironment({"EX3.x_hiding.x#int"})
      @FlatEnvironment({"x"})
      y_hiding xsy;
      x_hiding(){
        x = 2;
      ...
      @NestedEnvironment({"EX3.x_hiding.x#int", "EX3.x_hiding.xsy#y_hiding"})
      @FlatEnvironment({"x","xsy"})
    ...
    @NestedEnvironment({"EX3.x", "EX3.y"})
    @FlatEnvironment({"x","y"})
    int q; //should not recognize xsy
    static void func(){
      @Begin class Q {}
      EX3 top = new EX3();
  ...
  public static class EX4 { // Inheritance
    ...
    void func() {
      ...
      @NestedEnvironment({"EX4.x#int", "EX4.p#Parent", "EX4.c1#C1"})
      @FlatEnvironment({"x","p","c1"})
  ...
  public static class EX5 {
    ...
    class a{
      int a_x;
      @FlatEnvironment({"x", "a_x"})
      class b{
        int b_x;
        @FlatEnvironment({"x", "a_x", "b_x"})
        class c{
          int c_x;
          @FlatEnvironment({"x", "a_x", "b_x", "c_x"})
          class d{
            int d_x;
            @FlatEnvironment({"x", "a_x", "b_x", "c_x", "d_x"})
            void d_func(){
              @Begin class opening {/**/} 
              ++a_x;
              ++b_x;
  ...
  public static class EX6 {
    ...
    class Outer{
      int x;
      @NestedEnvironment({"EX6.Outer.x#int"})
      @FlatEnvironment({"x"})
      class Inner{
  ...
  public static class EX8 {
    class Arr{
      String[] arr;
      @NestedEnvironment({"EX8.Arr.arr#String[]"})
      @FlatEnvironment({"arr"})
      void foo(){
        @Begin class m {/**/}
        arr[2] = "$$$";
  ...
  public static class EX9 { // template
    public class SOList<Type> implements Iterable<Type> {
      ...
      @FlatEnvironment({"arrayList"})
      private int currentSize;
      @FlatEnvironment({"arrayList", "currentSize"})
      ...
      public Iterator<Type> iterator() {
        Iterator<Type> it = new Iterator<Type>() {
          @FlatEnvironment({"arrayList", "currentSize", "it"})
