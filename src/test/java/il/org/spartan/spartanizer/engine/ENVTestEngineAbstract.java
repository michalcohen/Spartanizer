...
public abstract class ENVTestEngineAbstract {
  ...
  private static boolean testSetContainsVarName(String s){
    for(Entry<String,Information> ¢ : testSet)
      if (¢.getKey().equals(s))
        return true;
    return false;
  }
   ...
   * First checks in unordered fashion, and then checks inorder.
   * @param ¢
   */
  ...
  private static void compareInOrder(final LinkedHashSet<Entry<String, Information>> contains, final LinkedHashSet<Entry<String, Information>> contained) {
    assert contained != null;
    assert contains != null;
    final Iterator<Entry<String, Information>> s = contains.iterator();
    for(Entry<String, Information> ¢ : contained){
      ...
      if(!entryFound){
        //Without the inner reset, one failing test will cause other, working, tests, to fail.
        testSetsReset();
  ...
  private static void compareOutOfOrder(final LinkedHashSet<Entry<String, Information>> contains, final LinkedHashSet<Entry<String, Information>> contained) {
    assert contains != null;
    assert contained != null;
    ...
    n.accept(new ASTVisitor() {
      ...
      void visitNodesWithPotentialAnnotations(final BodyDeclaration $) {
        ...
        if(userProvidedSet != null){
          //We're in manual mode, test against provided Set, not the generated one.
