...
@SuppressWarnings({ "unused" }) public abstract class FileTestUtils {
  ...
  static GUIApplicator makeLaconizationObject(final File ¢) {
    return makeLaconizationObject(¢.getName());
  }

  static GUIApplicator makeLaconizationObject(final String folderForClass) {
    final Class<?> c = asClass(folderForClass);
    assert c != null;
    final Object $ = getInstance(c);
    assert $ != null;
    return (GUIApplicator) $;
  }
  ...
  private static GUIApplicator error(final String message, final Class<?> c, final Throwable t) {
  ...
  public abstract static class Files extends FileTestUtils.Traverse {
    ...
    abstract Object[] makeCase(final GUIApplicator a, final File d, final File f, final String name);
  }
