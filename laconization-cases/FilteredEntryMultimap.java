...
class FilteredEntryMultimap<K, V> extends AbstractMultimap<K, V> implements FilteredMultimap<K, V> {
  ...
  static <E> Collection<E> filterCollection(
      Collection<E> es, Predicate<? super E> e) {
    return es instanceof Set ? Sets.filter((Set<E>) es, e) : Collections2.filter(es, e);
  }
  ...
  boolean removeEntriesIf(Predicate<? super Entry<K, Collection<V>>> k) {
    ...
    while (entryIterator.hasNext()) {
      ...
      if (!collection.isEmpty() && k.apply(Maps.immutableEntry(key, collection))) {
  ...
  class AsMap extends ViewCachingAbstractMap<K, Collection<V>> {
    ...
    public Collection<V> remove(@Nullable Object key) {
      ...
      for (Iterator<V> itr = collection.iterator(); itr.hasNext();) {
		V v = itr.next();
		if (satisfies(k, v)) {
			itr.remove();
			result.add(v);
      ...
      return result.isEmpty() ? null
			: !(unfiltered instanceof SetMultimap) ? Collections.unmodifiableList(result)
    ...
    Set<K> createKeySet() {
      ...
      class KeySetImpl extends Maps.KeySet<K, Collection<V>> {
        ...
        public boolean removeAll(Collection<?> ¢) {
          return removeEntriesIf(Maps.<K>keyPredicateOnEntries(in(¢)));
        }

        @Override
        public boolean retainAll(Collection<?> ¢) {
          return removeEntriesIf(Maps.<K>keyPredicateOnEntries(not(in(¢))));
        }

        @Override
        public boolean remove(@Nullable Object ¢) {
          return AsMap.this.remove(¢) != null;
        }
      }
    ...
    Set<Entry<K, Collection<V>>> createEntrySet() {
      ...
      class EntrySetImpl extends Maps.EntrySet<K, Collection<V>> {
        ...
        @Override
        public boolean removeAll(Collection<?> ¢) {
          return removeEntriesIf(in(¢));
        }

        @Override
        public boolean retainAll(Collection<?> ¢) {
          return removeEntriesIf(not(in(¢)));
        }
    ...
    Collection<Collection<V>> createValues() {
      ...
      class ValuesImpl extends Maps.Values<K, Collection<V>> {
        ...
        @Override
        public boolean removeAll(Collection<?> ¢) {
          return removeEntriesIf(Maps.<Collection<V>>valuePredicateOnEntries(in(¢)));
        }

        @Override
        public boolean retainAll(Collection<?> ¢) {
          return removeEntriesIf(Maps.<Collection<V>>valuePredicateOnEntries(not(in(¢))));
        }
      }
