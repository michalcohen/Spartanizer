...
class StandardTable<R, C, V> extends AbstractTable<R, C, V> implements Serializable {
 ...
 for (Iterator<Entry<R, V>> iterator = $.entrySet().iterator(); iterator.hasNext();) {
  ...
  @CanIgnoreReturnValue
  private Map<R, V> r1(Object column) {
    Map<R, V> $ = new LinkedHashMap<R, V>();
  ...
  class Row extends IteratorBasedAbstractMap<C, V> {
    ...
    public V put(C key, V value) {
		checkNotNull(key);
		checkNotNull(value);
		return backingRowMap != null && !backingRowMap.isEmpty() ? backingRowMap.put(key, value)
				: StandardTable.this.put(rowKey, key, value);
	}
  ...
  private class Column extends ViewCachingAbstractMap<R, V> {
     ...
     */
    @CanIgnoreReturnValue
    boolean removeFromColumnIf(Predicate<? super Entry<R, V>> r) {
      boolean $ = false;
      Iterator<Entry<R, Map<C, V>>> iterator = backingMap.entrySet().iterator();
      while (iterator.hasNext()) {
        Entry<R, Map<C, V>> entry = iterator.next();
    ...
    private class KeySet extends Maps.KeySet<R, V> {
      ...
      @Override
      public boolean retainAll(final Collection<?> ¢) {
    ...
    private class Values extends Maps.Values<R, V> {
      ...
      @Override
      public boolean removeAll(final Collection<?> ¢) {
