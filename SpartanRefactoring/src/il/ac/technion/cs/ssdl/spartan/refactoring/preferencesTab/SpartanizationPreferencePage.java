...
implements IWorkbenchPreferencePage {
  ...
  @Override public void createFieldEditors() {
    ...
    addField(new RadioGroupFieldEditor(bothLiteralsHeader, bothLiteralsHeader,
        1, BothLiteralsOptions, getFieldEditorParent()));
    addField(new RadioGroupFieldEditor(repositionRightLiteralsHeader,
        repositionRightLiteralsHeader, 1, rightLiteralOptions,
        getFieldEditorParent()));
    addField(new RadioGroupFieldEditor(nullAndBoolHeader, nullAndBoolHeader, 1,
        NullAndBoolOptions, getFieldEditorParent()));
    addField(new RadioGroupFieldEditor(swapMessageHeader, swapMessageHeader, 1,
        swapMessageOptions, getFieldEditorParent()));
  }

  @Override public boolean performOk() {
    super.performOk();
    SpartanizationPreferencePage.class.getProtectionDomain().getCodeSource()
    .getLocation().getPath();
    ...
    try (final PrintWriter print = new PrintWriter(
  ...
  @Override public void init(
      @SuppressWarnings("unused") final IWorkbench workbench) {
    super.initialize();
  }
}
