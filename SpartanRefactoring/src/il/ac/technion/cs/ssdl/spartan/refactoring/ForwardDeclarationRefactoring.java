	...
	protected ASTRewrite innerCreateRewrite(final CompilationUnit cu,
		...
		cu.accept(new ASTVisitor() {
			...
			public boolean visit(final VariableDeclarationFragment node) {
				...
				if (firstUseIdx<0)
					return true;
				final int beginingOfDeclarationsBlockIdx = findBeginingOfDeclarationBlock(
						block, declaredIdx, firstUseIdx);
				if (beginingOfDeclarationsBlockIdx > declaredIdx) {
					...
					if (((VariableDeclarationStatement)declarationNode).fragments().size()==1) {
						...
						rewrite.getListRewrite(block, Block.STATEMENTS_PROPERTY).
							insertAt(ast.newVariableDeclarationStatement((VariableDeclarationFragment)(ASTNode.copySubtree(ast, node)))
