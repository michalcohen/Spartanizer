...
/**
 * @author Ofir Elmakias <code><elmakias [at] outlook.com></code> 	(original / 24.05.2014)
 * @author Tomer Zeltzer <code><tomerr90 [at] gmail.com></code>		(original / 24.05.2014)
 * @since 2014/05/24
 */
public class ShortestOperand extends Spartanization {
	...
	@Override protected final void fillRewrite(final ASTRewrite r, final AST t,
		...
		cu.accept(new ASTVisitor() {
			...
			@Override public boolean visit(final InfixExpression n) {
				...
				r.replace(n, transpose(t,r,n) , null); // Replace old tree with the new organized one
				return true;
			}
	...
	/**
	 * Transpose infix expressions recursively.
	 * Makes the shortest operand first on every subtree of the node.
	 *
	 * @param ast
	...
	public static InfixExpression transpose(final AST ast,
		...
		if (newR.getNodeType() == BOOLEAN_LITERAL) return $; // Prevents the following swap: "(a>0) == true" => "true == (a>0)"

		if (longerFirst(n) && isFlipable(n.getOperator())){ // Swap between the operands
			$.setLeftOperand((Expression) newR);
			$.setRightOperand((Expression) newL);
			$.setOperator(flipOperator(n.getOperator()));
		}
	...
	/**
	 * Makes an opposite operator from a given one,
	 * which keeps its logical operation after the node swapping.
	 * e.g. "&" is commutative, therefore no change needed.
	 * 		"<" isn't commutative, but it has its opposite: ">=".
	 *
	 * @param o
	 * 		The operator to flip
	 * @return
	 * 		The correspond operator - e.g. "<=" will become ">", "+" will stay "+".
	 */
	 ...
	 * @return
	 * 		True - if the operator have opposite one
	 * 		in terms of operands swap.
	 * @see ShortestOperand
	 */
	public static boolean isFlipable(final Operator o) {
		if (o.equals(EQUALS) || o.equals(NOT_EQUALS) || o.equals(LESS) ||
				o.equals(GREATER) || o.equals(LESS_EQUALS) || o.equals(GREATER_EQUALS) ||
				o.equals(PLUS) ||  o.equals(TIMES) || o.equals(XOR) ||
				o.equals(AND) ||  o.equals(OR))
			return true;
		return false;
	}
	 ...
	 * 		Ranges to merge
	 * @return
	 * 		True - if such an overlap exists
	 * @see merge
	 */
	...
	/**
	 * Tries to union the given range with one of the elements
	 ...
	 * @return
	 * 		True - if the list updated and the new range consumed
	 * 		False - the list remained intact
	 *
	 * @see areOverlapped
	 * @see merge
	 */
	...
	@Override protected ASTVisitor fillOpportunities(
		...
		return new ASTVisitor() {
			@Override public boolean visit(final InfixExpression n) {
				...
				if (longerFirst(n) && isFlipable(n.getOperator())){	// Check if the operands can be swapped
					final boolean overlapFound = unionRangeWithList(opportunities, rN); // Union range results
					if (!overlapFound) opportunities.add(rN);
				}
	...
	static boolean longerFirst(final InfixExpression n) {
		return n.getLeftOperand() != null && n.getRightOperand() != null
				&& Funcs.countNodes(n.getLeftOperand()) > Funcs.countNodes(n.getRightOperand())
				+ threshold;
	}
