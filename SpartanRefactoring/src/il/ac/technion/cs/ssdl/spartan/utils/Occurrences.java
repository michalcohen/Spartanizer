...
public enum Occurrences {
	...
	USES_SEMANTIC {
		@Override
		ASTVisitor[] collectors(final List<Expression> into, final Expression e) {
	 ...
	 * @param e
	 *            what to search for
	 * @return a function object which can be used for searching for the
	 *         parameter in a given location
	 */
	public Of of(final Expression e) {
		return new Of() {
			@Override
			public List<Expression> in(final ASTNode n) {
				return collect(e, n);
			}
		};
	}
	 ...
	 * the value we search for; it represents the function that, given a
	 * location for the search, will carry out the search for the captured value
	 * in its location parameter.
	 *
	...
	abstract ASTVisitor[] collectors(final List<Expression> into,
			final Expression e);

	static ASTVisitor definitionsCollector(final List<Expression> into,
			final Expression e) {
		return new ASTVisitor() {
			@Override
			public boolean visit(final Assignment n) {
			...
			void collectExpression(final Expression candidate) {
				if (candidate != null
						&& candidate.getNodeType() == e.getNodeType()
	...
	static ASTVisitor lexicalUsesCollector(final List<Expression> into,
			final Expression what) {
		return usesCollector(into, what, true);
	}

	static ASTVisitor semanticalUsesCollector(final List<Expression> into,
			final Expression what) {
		return usesCollector(into, what, false);
	}

	private static ASTVisitor usesCollector(final List<Expression> into,
			final Expression what, final boolean lexicalOnly) {
		return new ASTVisitor() {
			...
			public boolean visit(final MethodDeclaration n) {
				/*
				 * Now: this is a bit complicated. Java allows declaring methods
				 * in anonymous classes in which the formal parameters hide
				 * variables in the enclosing scope. We don't want to collect
				 * them as uses of the variable
				 */
				for (final Object o : n.parameters())
					if (((SingleVariableDeclaration) o).getName().subtreeMatch(
							matcher, what))
						return false;
				return true;
			}
			...
			@Override
			public boolean visit(final AnonymousClassDeclaration n) {
			...
			@Override
			public boolean visit(final PrefixExpression n) {
			...
			@Override
			public boolean visit(final PostfixExpression n) {
				return collect(n.getOperand());
			}

			@Override
			public boolean visit(final ParenthesizedExpression n) {
			...
			@Override
			public boolean visit(final CastExpression n) {
				return collect(n.getExpression());
			}

			@Override
			public boolean visit(final ArrayAccess n) {
				return collect(n.getArray());
			}

			@Override
			public boolean visit(final MethodInvocation n) {
				collect(n.getExpression());
				return collect(n.arguments());
			}

			@Override
			public boolean visit(final ConstructorInvocation n) {
				return collect(n.arguments());
			}

			@Override
			public boolean visit(final ClassInstanceCreation n) {
				return collect(n.arguments());
			}

			@Override
			public boolean visit(final ArrayCreation n) {
				return collect(n.dimensions());
			}

			@Override
			public boolean visit(final ArrayInitializer n) {
				return collect(n.expressions());
			}

			@Override
			public boolean visit(final ReturnStatement n) {
				return collect(n.getExpression());
			}

			@Override
			public boolean visit(final FieldAccess n) {
				return collect(n.getExpression());
			}

			@Override
			public boolean visit(final QualifiedName n) {
			...
			@Override
			public boolean visit(final SwitchStatement n) {
				return collect(n.getExpression());
			}

			@Override
			public boolean visit(final InstanceofExpression n) {
				return collect(n.getLeftOperand());
			}

			@Override
			public boolean visit(final ForStatement n) {
				loopDepth++;
				return collect(n.getExpression());
			}

			@Override
			public boolean visit(final EnhancedForStatement n) {
				loopDepth++;
				return collect(n.getExpression());
			}

			@Override
			public boolean visit(final DoStatement n) {
				loopDepth++;
				return collect(n.getExpression());
			}

			@Override
			public boolean visit(final WhileStatement n) {
				loopDepth++;
				return collect(n.getExpression());
			}

			@Override
			public void endVisit(@SuppressWarnings("unused") final DoStatement _) {
				loopDepth--;
			}

			@Override
			public void endVisit(
					@SuppressWarnings("unused") final EnhancedForStatement _) {
				loopDepth--;
			}

			@Override
			public void endVisit(
					@SuppressWarnings("unused") final ForStatement _) {
				loopDepth--;
			}

			@Override
			public void endVisit(
					@SuppressWarnings("unused") final WhileStatement _) {
				loopDepth--;
			}
			...
			private List<VariableDeclarationFragment> getFieldsOfClass(
					final ASTNode classNode) {
				final List<VariableDeclarationFragment> $ = new ArrayList<>();
				classNode.accept(new ASTVisitor() {
					@Override
					public boolean visit(final FieldDeclaration n) {
						$.addAll(n.fragments());
						return false;
					}
				});
