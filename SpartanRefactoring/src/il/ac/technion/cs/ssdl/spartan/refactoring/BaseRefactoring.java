...
public abstract class BaseRefactoring extends Refactoring {
	...
	protected abstract ASTRewrite innerCreateRewrite(final CompilationUnit cu,
	...
	private void runAsManualCall(IProgressMonitor pm)
	 ...
	 */
	protected void scanCompilationUnits(ArrayList<ICompilationUnit> units, IProgressMonitor pm) throws IllegalArgumentException, CoreException {
	...
	protected void scanCompilationUnitForMarkerFix(IMarker m, IProgressMonitor pm, boolean preview)
			throws CoreException {
		...
		TextFileChange textChange = new TextFileChange(
				...
				(IFile) u.getResource());
		textChange.setTextType("java");
		textChange.setEdit(createRewriteForMarker(
				new SubProgressMonitor(pm, 1,SubProgressMonitor.SUPPRESS_SUBTASK_LABEL), m)
				.rewriteAST());
	...
	public static ICompilationUnit getCompilationUnitFromMarker(IMarker m) {
		final ICompilationUnit u = JavaCore
	...
	public RefactoringStatus initialize(final Map fArguments) {
		RefactoringStatus status = new RefactoringStatus();
		return status;
	}
}
