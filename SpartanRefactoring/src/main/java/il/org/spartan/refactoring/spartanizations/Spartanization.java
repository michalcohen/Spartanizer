...
public abstract class Spartanization extends Refactoring {
   ...
   */
  public static final List<ICompilationUnit> getCompilationUnits(final ICompilationUnit u, final IProgressMonitor pm) throws JavaModelException {
   ...
   * @throws JavaModelException
   */
  private static Void getCompilationUnits(final IProgressMonitor pm, final ICompilationUnit u, final List<ICompilationUnit> $) throws JavaModelException {
    if (u == null)
  ...
  protected static boolean isNodeOutsideMarker(final ASTNode n, final IMarker m) {
    try {
      return n.getStartPosition() < ((Integer) m.getAttribute(IMarker.CHAR_START)).intValue() || n.getLength() + n.getStartPosition() > ((Integer) m.getAttribute(IMarker.CHAR_END)).intValue();
  ...
  private List<ICompilationUnit> getUnits(final IProgressMonitor pm) throws JavaModelException {
    if (!isTextSelected())
      return getCompilationUnits(compilationUnit != null ? compilationUnit : BaseHandler.currentCompilationUnit(), new SubProgressMonitor(pm, 1, SubProgressMonitor.SUPPRESS_SUBTASK_LABEL));
    final List<ICompilationUnit> $ = new ArrayList<>();
    $.add(compilationUnit);
    return $;
  }
