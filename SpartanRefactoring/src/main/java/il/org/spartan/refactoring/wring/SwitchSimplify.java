...
public class SwitchSimplify extends ReplaceCurrentNode<SwitchStatement> implements Kind.SWITCH_IF_CONVERTION {
  ...
  @SuppressWarnings("unchecked") @Override ASTNode replacement(final SwitchStatement s) {
    ...
    Collections.sort(cc, (o1, o2) -> escapeLevel(bs.get(o1.get(0)).get(bs.get(o1.get(0)).size() - 1), false) - escapeLevel(bs.get(o2.get(0)).get(bs.get(o2.get(0)).size() - 1), false));
  ...
  @SuppressWarnings({ "javadoc" }) public static boolean containsSequencer(final Statement s, final boolean b) {
    return !b
        && Is.sequencer(s)
        || (s instanceof IfStatement ? containsSequencer(((IfStatement) s).getThenStatement(), b) && containsSequencer(((IfStatement) s).getElseStatement(), b) : s instanceof ForStatement
            && containsSequencer(((ForStatement) s).getBody(), true) || s instanceof WhileStatement && containsSequencer(((WhileStatement) s).getBody(), true) || s instanceof Block
            && !((Block) s).statements().isEmpty() && containsSequencer((Statement) ((Block) s).statements().get(((Block) s).statements().size() - 1), b));
  }
  @SuppressWarnings("javadoc") public static void insertUntilSequencer(final List<Statement> f, final List<Statement> ss, final int i) {
