...
import static il.org.spartan.refactoring.utils.Funcs.asExpressionStatement;
import static il.org.spartan.refactoring.utils.Funcs.asIfStatement;
import static il.org.spartan.refactoring.utils.Funcs.asMethodDeclaration;
import static il.org.spartan.refactoring.utils.Funcs.asMethodInvocation;
...
import static il.org.spartan.refactoring.utils.Funcs.asThrowStatement;
...
public enum Extract {
	...
	public static Expression core(final Expression $) {
		return $ == null || $.getNodeType() != PARENTHESIZED_EXPRESSION ? $
				: core(((ParenthesizedExpression) $).getExpression());
	}

	/**
	 * Computes the "essence" of a statement, i.e., if a statement is
	 * essentially a single, non-empty, non-block statement, possibly wrapped in
	 * brackets, perhaps along with any number of empty statements, then its
	 * essence is this single non-empty statement.
	 *
	 * @param s
	 *            JD
	 * @return the essence of the parameter, or <code><b>null</b></code>, if
	 *         there are no non-empty statements within the parameter. If,
	 *         however there are multiple non-empty statements inside the
	 *         parameter then the parameter itself is returned.
	 */
	 ...
	 * @return null if the statement is not an expression, nor a return
	 *         statement, nor a throw statement. Otherwise, the expression in
	 *         these.
	 */
	 ...
	 *
	 * @param n
	 *            a statement or a block to extract the expression statement
	 *            from
	 * @return the expression statement if n is a block or an expression
	 *         statement or null if it not an expression statement or if the
	...
	public static PostfixExpression findFirstPostfix(final ASTNode n) {
		...
		n.accept(new ASTVisitor() {
			@Override
			public boolean visit(final PostfixExpression e) {
				if ($.get() == null)
					$.set(e);
				return false;
			}
	...
	public static IfStatement firstIfStatement(final ASTNode n) {
		...
		n.accept(new ASTVisitor() {
			@Override
			public boolean visit(final IfStatement s) {
				if ($.get() == null)
					$.set(s);
				return false;
			}
	...
	public static MethodDeclaration firstMethodDeclaration(final ASTNode n) {
		...
		n.accept(new ASTVisitor() {
			@Override
			public boolean visit(final MethodDeclaration d) {
				if ($.get() == null)
					$.set(d);
				return false;
			}
	...
	public static InfixExpression firstPlus(final ASTNode n) {
		...
		n.accept(new ASTVisitor() {
			@Override
			public boolean visit(final InfixExpression e) {
				if ($.get() != null)
					return false;
	...
	public static Type firstType(final Statement s) {
		...
		s.accept(new ASTVisitor() {
			@Override
			public boolean preVisit2(final ASTNode n) {
	 ...
	 * @param n
	 *            JD
	 * @return the first such node encountered in a visit of the tree rooted a
	 *         the parameter, or <code><b>null</b></code>
	 */
	public static VariableDeclarationFragment firstVariableDeclarationFragment(final ASTNode n) {
		...
		n.accept(new ASTVisitor() {
			@Override
			public boolean visit(final VariableDeclarationFragment f) {
				if ($.get() == null)
					$.set(f);
				return false;
			}
	 ...
	 * @param n
	 *            JD
	 * @return the inner most {@link MethodDeclaration} in which the parameter
	 *         is nested, or <code><b>null</b></code>, if no such statement
	 *         exists.
	 */
	 ...
	 * @param n
	 *            JD
	 * @return the method invocation if it exists or null if it doesn't or if
	 *         the block contains more than one statement
	 */
	 ...
	 * @param n
	 *            JD
	 * @return the {@link IfStatement} that immediately follows the parameter,
	 ...
	 * @param n
	 *            JD
	 * @return the {@link ReturnStatement} that immediately follows the
	 ...
	 * @param s
	 *            JD
	 * @return the single statement in the <code><b>else</b></code> branch of
	 *         the parameter, or <code><b>null</b></code>, if no such statement
	 *         exists.
	 */
	 ...
	 * @param n
	 *            JD
	 * @return if b is a block with just 1 statement it returns that statement,
	 ...
	 * @param n
	 *            JD
	 * @return the inner most {@link Statement} in which the parameter is
	...
	public static List<IExtendedModifier> modifiers(TypeDeclaration d) {
		return d.modifiers();
	}

  public static List<IExtendedModifier> modifiers(ASTNode ¢) {
